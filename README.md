...
- **Firebase設定連携**:
   - Geminiモデル名とプロンプトテンプレートを Firestore `settings/app` ドキュメントで一元管理できます。
   - 生成待ちの間は進捗バーと処理段階メッセージを表示します。
...
### 5. 📋 監査ログ（Audit Logs）
- 主要操作（ログイン/ログアウト、生成開始/成功/失敗、保存成功/失敗、設定更新）を Firestore `auditLogs` コレクションに記録します。
...
```env
VITE_GEMINI_API_KEY=あなたのGEMINI_API_KEY
```
